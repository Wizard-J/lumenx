---
name: "source-command-lumenx-git-publish"
description: "LumenX 个人 fork 发布流程 - 安全提交、敏感数据扫描、推送到个人 GitHub 仓库"
---

# source-command-lumenx-git-publish

Use this skill when the user asks to run the migrated source command `lumenx-git-publish`.
...
```bash
grep -E "^\.env|^\.agent|^Codex\.md|^output/" .gitignore
```

确保至少包含：`.env`、`.agent/`、`AGENTS.md`、`output/`
