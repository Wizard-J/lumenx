...
import { Check, Copy, Image as ImageIcon, Loader2, Lock, Plus, Trash2, Unlock, Upload, WandSparkles, X } from "lucide-react";
...
export default function AssetStageDialog({ open, asset, assetType, currentEpisode, onClose, onAction }: Props) {
  ...
  // Auto-create a default base stage when no stages exist yet
  useEffect(() => {
    if (!open) { autoCreateStartedRef.current = false; return; }
    if (!onAction) return;
    ...
    setBusy("create");
    setError(null);
    onAction(asset, "create", undefined, {
      label: "基础阶段",
      from_episode: 1,
      to_episode: Math.max(currentEpisode ?? 1, 12),
      visual_delta: "",
    }).catch((cause: any) => {
      setError(cause?.response?.data?.detail || cause?.message || "创建基础阶段失败");
      autoCreateStartedRef.current = false;
    }).finally(() => setBusy(null));
  }, [open, onAction, asset, stages.length, currentEpisode]);
  ...
  useEffect(() => {
    ...
    setGenerationPrompt(selected.last_generation_prompt || effectivePrompt);
  }, [effectivePrompt, promptDirty, selected]);
  if (!open) return null;
  ...
  const uploadStageImage = async (file: File | undefined) => {
    if (!file || !selected || !onAction) return;
    setBusy("upload"); setError(null);
    try {
      const result = await api.uploadFile(file);
      await onAction(asset, "use_image", selected, {
        image_url: result.url,
        prompt_used: `用户上传：${file.name || "阶段参考图"}`,
      });
    } catch (cause: any) {
      setError(cause?.response?.data?.detail || cause?.message || "上传阶段参考图失败");
    } finally {
      setBusy(null);
      ...
      <div className="glass-panel w-full max-w-5xl overflow-hidden rounded-2xl border border-white/10 bg-[#09090f]/95 shadow-2xl">
          ...
          <div className="flex flex-col items-center gap-3 px-5 py-16 text-center">
            ...
            {busy === "create" ? (
              <>
                ...
                <div>
                  <p className="text-sm text-foreground">正在准备基础阶段...</p>
                  <p className="mt-1 text-xs text-text-muted">准备完成后即可生成或上传参考图。</p>
                </div>
              </>
            ) : (
              <>
                <div><p className="text-sm text-foreground">还没有阶段资产</p><p className="mt-1 text-xs text-text-muted">创建后即可从现有参考图中指定该阶段使用的图片。</p></div>
                {error ? (
                  <button disabled={!!busy} onClick={() => run("create", { label: "基础阶段", from_episode: 1, to_episode: Math.max(currentEpisode ?? 1, 12), visual_delta: "" }, undefined)} className="glass-button text-xs text-primary">重试创建基础阶段</button>
                ) : null}
              </>
            )}
          </div> :
        <div className="grid max-h-[72vh] gap-5 overflow-y-auto p-5 md:grid-cols-[300px_1fr]">
          ...
          <div className="min-w-0 space-y-4">
            ...
            <div className="rounded-lg border border-primary/15 bg-primary/[.04] p-3"><div className="mb-2 flex items-center justify-between gap-3"><div className="text-[10px] font-mono uppercase tracking-wider text-primary">阶段生成提示词</div><button type="button" disabled={!!busy} onClick={() => { setPromptDirty(false); setGenerationPrompt(effectivePrompt); }} className="text-[10px] text-text-muted transition hover:text-text-secondary disabled:opacity-40">恢复默认</button></div><textarea aria-label="阶段生成提示词" value={generationPrompt} onChange={(event) => { setGenerationPrompt(event.target.value); setPromptDirty(true); }} className="glass-input min-h-28 w-full resize-y p-3 text-xs leading-5 text-text-secondary"/><p className="mt-1 text-[10px] text-text-muted">{promptDirty ? "将提交你编辑后的提示词" : selected.last_generation_prompt ? "上次实际提交；可直接修改后重试" : "将按当前内容提交"}</p></div>
            ...
            <div className="flex flex-wrap gap-2"><button disabled={!!busy} onClick={() => selected.locked ? setError("当前阶段已锁定，请先解锁后再生成。") : run("generate", {prompt:generationPrompt.trim() || effectivePrompt, batch_size:batchSize, aspect_ratio:aspectRatio})} className={`glass-button flex items-center gap-2 text-xs ${selected.locked ? "text-text-muted" : "text-primary"}`}>{busy === "generate" ? <Loader2 size={14} className="animate-spin"/> : <WandSparkles size={14}/>}生成当前阶段 ×{batchSize}</button><input ref={uploadInputRef} type="file" accept="image/*" className="hidden" onChange={(event) => uploadStageImage(event.target.files?.[0])}/><button disabled={!!busy || selected.locked} onClick={() => selected.locked ? setError("当前阶段已锁定，请先解锁后再上传。") : uploadInputRef.current?.click()} className="glass-button flex items-center gap-2 text-xs text-text-secondary hover:text-foreground">{busy === "upload" ? <Loader2 size={14} className="animate-spin"/> : <Upload size={14}/>}上传参考图</button><button disabled={!!busy} onClick={() => run("copy_previous")} className="glass-button flex items-center gap-2 text-xs"><Copy size={14}/>复用上一阶段</button>{candidates.length > 0 && <button disabled={!!busy} onClick={() => run("use_base")} className="glass-button text-xs">使用当前基础主图</button>}<button disabled={!!busy} onClick={() => run("toggle_lock")} className="glass-button ml-auto flex items-center gap-2 text-xs">{selected.locked ? <Unlock size={14}/> : <Lock size={14}/>} {selected.locked ? "解锁" : "锁定"}</button></div>
          </div>
        </div>}
      </div>
    </div>
